...
import type { SACCEPTANCE_ID, SCONSTRAINT_ID, SDOMAIN_ID, SFEATURE_ID, SRELATION_ID, SREQUIREMENT_ID } from "../specification/artifact.js";
import type { OAPP_ID, OCONCEPT_ID, ODECISION_ID, OMODULE_ID, OPACKAGE_ID, ORELATION_ID, ORESOURCE_ID, OSCENARIO_ID } from "../solution/artifact.js";
...
export type EVIDENCE = { // evidence 不可复用，每个证据只能用于一个检查
  id: VEVIDENCE_ID;
  ...
  target_ref:
    | SDOMAIN_ID | SFEATURE_ID | SREQUIREMENT_ID | SCONSTRAINT_ID | SRELATION_ID | SACCEPTANCE_ID
    | OAPP_ID | OPACKAGE_ID | OMODULE_ID | ORESOURCE_ID | ORELATION_ID | OSCENARIO_ID | OCONCEPT_ID | ODECISION_ID;
...
export interface ValidationArtifact {
  ...
  specification_checks: { // 实现满足了哪些需求，哪些不满足，哪些尚未充分验证
    target:
    | {
      type: "domain"
      ref: SDOMAIN_ID
  ...
  solution_checks: { // 实现是否遵守设计职责、接口、协作和共同规则，具体偏离在哪里？
    target:
    | {
      type: "app"
      ref: OAPP_ID
  ...
  tests_checks: {
    ...
    ttarget_refs: (SREQUIREMENT_ID | SCONSTRAINT_ID | SRELATION_ID | SACCEPTANCE_ID | OSCENARIO_ID | OCONCEPT_ID | ODECISION_ID)[];
