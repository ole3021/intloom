...
import type { SCONSTRAINT_ID, SREQUIREMENT_ID } from "../specification/artifact.js";
...
export type PARTICIPANT_TYPE = "module" | "resource" | "actor" | "external_system";
...
export interface SolutionArtifact {
    ...
    structure: {
        apps: {
            ...
            technologies?: { // 只包含架构上重要的技术,也就是换掉它,solution 必须重新设计才放,其它实现细节一律不放
        ...
        resources: {
            ...
            capabilities?: { // 只有会影响 Solution 的关键能力才记录。
                name: string;
                description?: string;
            }[];
            requirement_refs?: SREQUIREMENT_ID[];
    ...
    scenarios: { // 每个场景有目标、参与方和步骤,步骤里写谁和谁交互,以及交互的描述
        id: OSCENARIO_ID;
        status: LIFE_CYCLE_STATUS;
    ...
    concepts: { // 跨 Module / Scenario 需要共同遵守、共同理解的设计概念与规则。
        id: OCONCEPT_ID;
        name: string;
        status: LIFE_CYCLE_STATUS;
        ...
        rules: { // 所有适用对象必须共同遵守的设计规则。
            id: OCONCEPT_RULE_ID;
            description: string;
        }[];
        record_refs: RUN_ID[];
    }[]
    decisions: { // 最终决定了什么、为什么、影响哪里、由哪些需求/约束驱动
        id: ODECISION_ID;
        name: string;
        status: LIFE_CYCLE_STATUS;
        ...
        scope_refs?: (SOLUTION_STRUCTURE_REFS | ORELATION_ID | OSCENARIO_ID | OCONCEPT_ID)[];
    ...
    risks: { //写风险和作用范围,以及风险的影响和缓解措施
        ...
        scope_refs?: (SOLUTION_STRUCTURE_REFS | ORELATION_ID | OSCENARIO_ID | OCONCEPT_ID | ODECISION_ID)[];
    ...
    diagrams: { //一个总体架构图、单个 app 或 service 的组件图、场景流程图,还有状态图
        id: ODIAGRAM_ID;
        name: string;
        status: LIFE_CYCLE_STATUS;
        type: DIAGRAM_TYPE;
        ...
        refs: (SOLUTION_STRUCTURE_REFS | ORELATION_ID | OSCENARIO_ID | OCONCEPT_ID | ODECISION_ID | ORISK_ID)[];
